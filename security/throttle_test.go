...
// TestIPThrottlerStopEndsCleanupGoroutine covers issue #20. cleanup ran
// `for range ticker.C` with no stop channel and no Stop method, so every
// throttler leaked a goroutine and a ticker for the life of the process.
func TestIPThrottlerStopEndsCleanupGoroutine(t *testing.T) {
	before := runtime.NumGoroutine()

	throttlers := make([]*IPThrottler, 0, 20)
	for i := 0; i < 20; i++ {
		throttlers = append(throttlers, NewIPThrottler(DefaultThrottleConfig()))
	}

	for _, th := range throttlers {
		th.Stop()
		th.Stop() // must be idempotent
	}

	assert.Eventually(t, func() bool {
		return runtime.NumGoroutine() <= before+2
	}, 5*time.Second, 50*time.Millisecond,
		"cleanup goroutines leaked: %d before, %d after Stop", before, runtime.NumGoroutine())
}

// TestBlacklistingSpawnsNoGoroutines covers the other half: blacklisting used
// to start a goroutine that slept out the penalty -- up to two hours,
// uncancellable, one per event, at a rate the attacker controls.
func TestBlacklistingSpawnsNoGoroutines(t *testing.T) {
	config := DefaultThrottleConfig()
	config.EnableSuspiciousDetection = true
	config.SuspiciousThreshold = 2
	config.SuspiciousPenaltyMinutes = 60
	...
	for i := 0; i < 50; i++ {
		req := httptest.NewRequest("GET", "/test", nil)
		req.RemoteAddr = fmt.Sprintf("203.0.113.%d:5555", i%256)
		for j := 0; j < 5; j++ {
			throttler.RecordFailure(req, 404)
		}
	}
	...
	assert.LessOrEqual(t, runtime.NumGoroutine(), before+2,
		"blacklisting spawned goroutines; it should just record a deadline")
}
